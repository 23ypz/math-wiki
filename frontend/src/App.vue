  ...
  <div class="app-shell" :class="{ 'sidebar-collapsed': sidebarCollapsed, 'guest-mode': guestMode }">
    ...
    <aside v-if="isLoggedIn()" class="sidebar" :class="{ 'mobile-open': mobileOpen }">
      <div class="sidebar-top">
        <div class="brand"><div class="brand-mark">数</div><div class="brand-copy"><h1>Math Wiki</h1><p>YOUR STUDY OS</p></div><button class="collapse-button" @click="toggleSidebar">{{ sidebarCollapsed ? '›' : '‹' }}</button><button class="drawer-close" aria-label="关闭导航" @click="mobileOpen = false">×</button></div>
        <RouterLink to="/profile" class="sidebar-profile">
          <div class="mini-avatar" :class="`avatar-${profile?.avatar_style || 'blue'}`"><img v-if="profile?.avatar_url" :src="profile.avatar_url" alt="头像"><span v-else>{{ initials }}</span></div>
